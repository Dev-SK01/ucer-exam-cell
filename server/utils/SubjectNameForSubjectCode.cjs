    ...
    for (j = 0; j <pdf.Pages.length; j++) {
        ...
        for (i = 0; i <= pdf.Pages[j].Texts.length - 1; i++) {
            ...
            if(pdf.Pages[j].Texts[i].R[0].T==="Code"){
                ...
                for(let sub=PositionForSubjectCode;sub<=EndPositionForSubjectCode;sub++){
                    let SubjectNameWithoutUnwantedChar=await ReplacingChar(pdf.Pages[j].Texts[PositionForSubjectName].R[0].T)
                    SubjectName[pdf.Pages[j].Texts[sub].R[0].T]=SubjectNameWithoutUnwantedChar
...
module.exports =SubjectNameForSubjectCode;
